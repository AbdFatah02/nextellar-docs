...
Keep the selected network in application state and remount the provider when it changes. Remounting clears the previous network's wallet context so child hooks use the new Horizon endpoint and network passphrase together.

```tsx
'use client';

import { useState } from 'react';
import { WalletProvider, useWalletConfig } from '@/contexts';
...
function NetworkStatus() {
  ...
  return <p>Connected to {config?.network ?? 'TESTNET'}</p>;
}

export default function App() {
  const [selectedNetwork, setSelectedNetwork] =
    useState<keyof typeof networks>('testnet');
  ...
  return (
    <>
      ...
      <WalletProvider key={selectedNetwork} {...config}>
        <NetworkStatus />
      </WalletProvider>
    </>
  );
