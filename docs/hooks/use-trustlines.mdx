...
<Note>
  Using a different wallet? Replace step 2 with that wallet's signing call. Steps 1 and 3 stay the
  same because `buildChangeTrustXDR` and `submitSignedXDR` only deal in XDR strings.
</Note>
