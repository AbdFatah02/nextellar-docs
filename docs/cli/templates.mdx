...
Add `-j` or `--javascript` to scaffold `.jsx` and `.js` files instead of TypeScript. All three templates include a JavaScript variant:
...
<Note>
  Pass `default`, `minimal`, or `defi` to `--template`. Names such as `js-defi`
  identify internal template directories and aren't valid flag values.
</Note>

See [Using JavaScript](/docs/getting-started/javascript) for language differences. See [Create a JavaScript DeFi project](/docs/cli/js-defi) for the DeFi template's features and setup.
